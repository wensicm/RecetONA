...
class OpenAIBackend:
    ...
    def require_client(self) -> OpenAI:
        if self.client is None:
            raise RuntimeError(
                "Falta OPENAI_API_KEY. Configura OPENAI_API_KEY o RECETONA_OPENAI_API_KEY."
            )
        return self.client
    ...
    def _retry(self, operation_name: str, callback):
        ...
        for attempt in range(1, self.settings.openai_max_retries + 1):
            try:
                ...
                if attempt >= self.settings.openai_max_retries:
                    break
                wait_seconds = self.settings.openai_retry_base_seconds * (
                    2 ** (attempt - 1)
                )
                LOGGER.warning(
        ...
        raise RuntimeError(
            f"OpenAI {operation_name} failed: {last_error}"
        ) from last_error
    ...
    def generate_text(self, prompt: str, model: str | None = None) -> str:
        ...
        reasoning_effort = self.settings.reasoning_effort.strip().lower()
        if reasoning_effort:
